...
protected:
  /*
    ...
    in a NEW object;
  */
  virtual Gaussian sum(float _mean, float _variance){
    float nMean = (variance * _mean + _variance * mean) / (_variance + variance);
    float nVariance = 1.0 / (1.0/variance + 1.0/_variance);
...
public:
  ...
  */
  Gaussian(float _mean = 0.0, float _variance = MAX_VARIANCE){
    mean = _mean;
    variance = _variance;
  }
  ...
  */
  virtual Gaussian setMean(float _val){
    mean = _val;
  ...
  */
  virtual Gaussian move(float _val){
    mean += _val;
  ...
  */
  virtual Gaussian vary(float _val){
    variance += _val;
  ...
  /*
    Increase the variance by _val
  */
  virtual Gaussian setVariance(float _val){
    variance = _val;
  ...
  */
  virtual float random(){
    float R1, R2;
    R1 = ::random(MAX_VARIANCE)/(float)MAX_VARIANCE;
    R2 = ::random(MAX_VARIANCE)/(float)MAX_VARIANCE;
    return mean + variance * cos( 2 * M_PI * R1) * sqrt(-log(R2));
  }

  virtual float plot(float x){
